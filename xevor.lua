...
local ConfigurationFolder = xevorFolder.."/Configurations"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("xevor | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, xevorFolder) then
				if callSafely(isfile, xevorFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, xevorFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('xevor had an issue accessing configuration saving capability.')
		end
	end
end
...
local xevor = useStudio and script.Parent:FindFirstChild('xevor') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	...
	if not warned then
		warn('xevor | Build Mismatch')
		print('xevor may encounter issues as you are running an incompatible interface version ('.. ((xevor:FindFirstChild('Build') and xevor.Build.Value) or 'No Build') ..').\n\nThis version of xevor is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, xevor = xevor, useStudio and script.Parent:FindFirstChild('xevor') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(xevor)
	xevor.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	xevor.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == xevor.Name and Interface ~= xevor then
			Interface.Enabled = false
			Interface.Name = "xevor-Old"
		end
	end
elseif not useStudio then
...
if xevor.AbsoluteSize.X < minSize.X and xevor.AbsoluteSize.Y < minSize.Y then
...
local Main = xevor.Main
local MPrompt = xevor:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = xevor:FindFirstChild('Drag')
...
xevor.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/xevor/refs/heads/main/icons.lua')
-- Variables
...
local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = xevorLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	xevor.Main.BackgroundColor3 = SelectedTheme.Background
	xevor.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	xevor.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	xevor.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	xevor.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	xevor.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	xevor.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		xevor.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		xevor.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(xevor:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("xevor | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("xevor | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('xevor had an issue decoding the configuration file, please try delete the file and reopen xevor.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(xevorLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("xevor | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--xevorLibrary:Notify({Title = "xevor Flags", Content = "xevor was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(xevorLibrary.Flags) do
...
function xevorLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('xevor | Not seeing your actions in notifications?')
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, xevorFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('xevor Settings', 0, true)

	if TabList['xevor Settings'] then
		TabList['xevor Settings'].LayoutOrder = 1000
	end

	if Elements['xevor Settings'] then
		Elements['xevor Settings'].LayoutOrder = 1000
	end
...
function xevorLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				xevorLibrary:Notify({Title = 'Build Mismatch', Content = 'xevor may encounter issues as you are running an incompatible interface version ('.. ((xevor:FindFirstChild('Build') and xevor.Build.Value) or 'No Build') ..').\n\nThis version of xevor is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "xevor"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "xevor Interface Suite" then
		LoadingFrame.Version.Text = "xevor UI"
	end
	...
	if not Settings.DisablexevorPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				xevorLibrary:Notify({
					Title = "xevor Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(xevorFolder.."/Discord Invites")

		if callSafely(isfile, xevorFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, xevorFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"xevor RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(xevorFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("xevor | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with xevor specific development.')
				end
			end
		end
		...
		if callSafely(isfile, xevorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, xevorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, xevorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						xevorLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			TweenService:Create(Button.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()	
				...
				if not Success then
					...
					print("xevor | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with xevor specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= xevorLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					xevorLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			xevor.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("xevor | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with xevor specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					xevorLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("xevor | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with xevor specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					xevorLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("xevor | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with xevor specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					xevorLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= xevorLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("xevor | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with xevor specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						xevorLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("xevor | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with xevor specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					xevorLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			xevorLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			xevorLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('xevor had an issue creating settings.') end
...
function xevorLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function xevorLibrary:IsVisible(): boolean
	return not Hidden
end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['xevor Settings'])
		end)
	end)
...
function xevorLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				xevorLibrary:Notify({Title = "xevor Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			xevorLibrary:Notify({Title = "xevor Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('xevor Configurations Error | '..tostring(result))
			xevorLibrary:Notify({Title = "xevor Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = xevorLibrary:CreateWindow({
	--	Name = "xevor Example Window",
	--	LoadingTitle = "xevor Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using xevor may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like xevor to get the key from
	...
	----xevorLibrary:Notify({Title = "xevor Interface", Content = "Welcome to xevor. These - are the brand new notification design for xevor, with custom sizing and xevor calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(xevorLibrary.Theme) do
...
task.delay(4, function()
	xevorLibrary.LoadConfiguration()
